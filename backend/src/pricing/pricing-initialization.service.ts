...
@Injectable()
export class PricingInitializationService implements OnModuleInit {
  private readonly logger = new Logger(PricingInitializationService.name);
  ...
  private async initializeTestMaster() {
    ...
    const tests = [
      ...
      // ── RADIOLOGY — X-Ray — 170 records ──
      ...this.getXrayRecords(),

      // ── RADIOLOGY — USG — 24 records ──
      ...
      // ── RADIOLOGY — Mammography — 2 records ──
      ...
      // ── PHYSICIAN CONSULTATION — 7 records (REMOVED AS PER USER REQUEST) ──

      // ── PHYSIOTHERAPY — 4 records ──
  ...
  // ===========================================================================
  // X-RAY RECORDS — 170 records (helper method)
  // ===========================================================================
  private getXrayRecords() {
    const xrays: Array<{ name: string; price: number }> = [
  ...
  // ===========================================================================
  // SERVICE MASTER — 12 records
  // ===========================================================================
  private async initializeServiceMaster() {
    ...
    for (let i = 0; i < services.length; i++) {
      ...
      await this.prisma.serviceMaster.upsert({
        where: { department: service.department as any },
  ...
  // ===========================================================================
  // PACKAGES — 9 records + PackageItems
  // ===========================================================================
  private async initializePackages() {
    ...
    for (let i = 0; i < packages.length; i++) {
      ...
      // Delete existing package items and re-create
    ...
    const doctors = [
      {
        ...
        specialization: ['DOCTOR_CONSULTATION'] as any[],
