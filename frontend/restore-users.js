...
async function main() {
  ...
  const data = JSON.parse(fs.readFileSync('../backend/db-export.json', 'utf8'));
  for (const u of data.users) {
    if (u.email === 'admin@eshs.in') continue; // already inserted
    try {
      await client.query(
        'INSERT INTO "users" ("id", "email", "password", "first_name", "last_name", "role", "is_active", "updated_at") VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())',
