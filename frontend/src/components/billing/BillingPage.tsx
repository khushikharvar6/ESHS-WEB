...
import { Plus, Search, X, Printer, FileText, Save, ChevronLeft, Info, Trash2, Share2 } from 'lucide-react'
...
const REFER_TYPES = ['Walk-in', 'Doctor Referral', 'Corporate Tie-up', 'Camp / Outreach'] as const
const DOCTORS = ['Dr. Anand Gadhvi', 'Dr. Nidhi Desai', 'Dr. Vidhi Shukla', 'Dr. Narendra Shah', 'Dr. Jyoti Shah'] as const
const REFERENCE_BY_OPTIONS = ['Doctor', 'Camps/community Activity', 'Corporate Tie-up', 'Insurance/TPA', 'Existing Patient'] as const
...
export function BillingPage() {
  ...
  const { refreshData } = useHealthcare()
  ...
  const [referredBy, setReferredBy] = useState('Doctor')
  ...
  const [dbServices, setDbServices] = useState<any[]>(SERVICE_MASTER)
  const [dbPackages, setDbPackages] = useState<any[]>(PACKAGE_MASTER)

  useEffect(() => {
    billingApi.getPricingItems().then(data => {
      if (data.tests?.length) setDbTests(data.tests.map((t: any) => ({ ...t, itemType: 'TEST' })))
      if (data.services?.length) setDbServices(data.services.map((s: any) => ({ ...s, itemType: 'SERVICE' })))
      if (data.packages?.length) setDbPackages(data.packages.map((p: any) => ({ ...p, itemType: 'PACKAGE' })))
    }).catch(console.error)
  }, [])
  ...
  useEffect(() => {
    billingApi.getPatients().then((data) => {
      setPatients(data)
      if (initialUhid) {
        const found = data.find((p: any) => p.uhid === initialUhid)
        if (found) {
          setPatient(found)
          setPatientSearch(found.name)
          setInvoice(prev => ({ ...prev, patient: found, updatedAt: new Date().toISOString() }))
        }
      }
    }).catch(() => {})
  }, [initialUhid])
  ...
  // Automatically assume full payment unless user explicitly alters it (user expectation)
  useEffect(() => {
    setReceivedAmt(totals.grandTotal)
  }, [totals.grandTotal])
  ...
  const filteredServices = useMemo(() => {
    ...
    if (billingType === 'Consultation') {
      return dbServices.filter(s =>
        ...
        (!department || s.department === department || (s.department && s.department.replace('_', ' ').toLowerCase() === department.toLowerCase()))
      )
    }

    if (billingType === 'Package' || serviceType === 'OP package') {
      return dbPackages.filter(p =>
        p.isActive !== false &&
        p.name.toLowerCase().includes(q)
      )
    }

    if (billingType === 'Services') {
      const srvs = dbServices.filter(s =>
        s.isActive !== false &&
        s.name.toLowerCase().includes(q) &&
        (!department || s.department === department || (s.department && s.department.replace('_', ' ').toLowerCase() === department.toLowerCase()))
      )
      const tsts = dbTests.filter(t =>
        t.isActive !== false &&
        t.name.toLowerCase().includes(q) &&
        (!department || t.department === department || (t.department && t.department.replace('_', ' ').toLowerCase() === department.toLowerCase())) &&
        (!serviceType || t.serviceType === serviceType || t.category === serviceType)
      )
      return [...srvs, ...tsts].slice(0, 50)
    }
  ...
  }, [billingType, serviceSearch, department, serviceType, dbServices, dbPackages, dbTests])
  ...
  const addServiceItem = (item: { id: string; name: string; price: number; taxRate: number; category: string; department: string; itemType: string; serviceType?: string; subcategory?: string; serviceCode?: string }) => {
    ...
    const invoiceItem: BillingInvoiceItem = {
      ...
      department: item.itemType === 'PACKAGE' ? 'Health Checkup' : item.department,
      quantity: 1,
      ...
      performBy: '',
      subDepartment: item.itemType === 'PACKAGE' ? 'Health Checkup' : (item.serviceType || item.subcategory || item.category || 'General'),
      servicePriority: 'Routine',
      orderId: `ORD-${Math.floor(100000 + Math.random() * 900000)}`,
      confirmOrder: 'Confirmed',
      patientAmount: Number(item.price) || 0,
      discount: 0,
      payerAmount: 0,
      emergencyCharges: 0,
      emergencyPercent: 0,
  ...
  // Create Bill
  const createBill = async (newWindow?: Window | null) => {
    if (!patient) { toast.error('Please select a patient first'); if (newWindow) newWindow.close(); return }
    if (invoice.items.length === 0) { toast.error('Please add at least one service'); if (newWindow) newWindow.close(); return }
    ...
    try {
      ...
      if (patient.phone) {
        import('@/lib/sms').then(({ generateWhatsAppLink, NotificationTemplates }) => {
          const msg = NotificationTemplates.invoiceGenerated(patient.name, result.invoiceNumber ?? result.id, totals.grandTotal)
          const waUrl = generateWhatsAppLink(patient.phone, msg)
          if (newWindow) {
  ...
  const saveAndShare = async () => {
    ...
    // We already call createBill which also opens whatsapp if phone exists
    await createBill(newWindow);
  }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <div className="print:hidden flex flex-col flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div>
              ...
              <select className="w-full border border-slate-300 rounded px-2 py-1.5 text-sm bg-white" value={referredBy} onChange={e => setReferredBy(e.target.value)}>
                {REFERENCE_BY_OPTIONS.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            </div>
            {/* Treating Doctor */}
            <div>
                      ...
                      <tr key={item.id} className={`border-b border-slate-100 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-blue-50/40 transition-colors`}>
                        ...
                        <td className="px-1 py-1">
                          <select className="border border-slate-300 rounded px-1 py-0.5 text-xs bg-white" value={item.itemType} onChange={e => updateItemField(item.id, 'itemType', e.target.value)}>
                            ...
                            <option value="PACKAGE">OP package</option>
                          </select>
                        </td>
                        ...
                        {/* Perform By */}
                        <td className="px-1 py-1">
                          <input type="text" className="border border-slate-300 rounded px-1 py-0.5 text-xs bg-white w-28" value={item.performBy || ''} onChange={e => updateItemField(item.id, 'performBy', e.target.value)} />
                        </td>
                        {/* Qty */}
        ...
        <div className="bg-white border-t border-slate-200 px-4 py-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {activePaymentTab === 1 && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm items-center">
                    ...
                    <select className="border border-slate-300 rounded px-2 py-1 text-sm bg-white" value={invoice.discountType} onChange={e => setInvoice(prev => ({...prev, discountType: e.target.value as 'flat' | 'percentage', updatedAt: new Date().toISOString()}))}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    {paymentMode !== 'Cash' && paymentMode !== 'Insurance' && (
                      <>
                        ...
                        <input type="text" className="border border-slate-300 rounded px-2 py-1 text-sm bg-white" placeholder="e.g. HDFC Bank" value={bankName} onChange={e => setBankName(e.target.value)} />
                        <span className="text-slate-500">Transaction ID</span>
                        <input type="text" className="border border-slate-300 rounded px-2 py-1 text-sm bg-white" placeholder="Transaction No." value={transactionId} onChange={e => setTransactionId(e.target.value)} />
                      </>
                    )}
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <span className="text-slate-500">Discount ({invoice.discountType === 'percentage' ? '%' : 'Amt'})</span>
              <input type="number" className="border border-slate-300 rounded px-2 py-1 text-sm bg-white" value={discountAmt} onChange={e => setDiscountAmt(Number(e.target.value))} />
      ...
      <div id="printable-invoice" className="hidden print:block font-serif text-[11px] text-black bg-white px-8 pt-8 pb-32">
        {/* Printable Header */}
        <div className="flex items-center justify-between mb-4">
          {/* Green left banner */}
          <div className="w-8 h-16 bg-gradient-to-b from-[#5c9868] to-[#88c575]" />
          {/* Centered logo */}
        ...
        <div className="border-y-[3px] border-black py-2 mb-4">
          <div className="grid grid-cols-2 gap-x-8 text-[11px] font-serif">
            <div className="grid grid-cols-[100px_1fr] gap-y-1">
              <span className="font-bold">UHID</span><span>{patient?.uhid || 'ESHS2025-15550'}</span>
              <span className="font-bold">Patient Name</span><span>{patient?.name || 'Ms Khushi Jayeshkumar Kharvar'}</span>
              <span className="font-bold leading-tight">Contact<br/>Number</span><span className="flex items-center">{patient?.phone || '9904263460'}</span>
              <span className="font-bold leading-tight">Consultant<br/>Name</span><span className="flex items-center">{treatingDoctor}</span>
        ...
        {/* OP Bill Title */}
        <div className="text-center font-serif text-base tracking-wide my-4 uppercase">OP BILL</div>

        {/* Printable Items Table */}
        <table className="w-full text-left text-[11px] mb-6 border-collapse">
          <thead>
            <tr>
              <th className="py-1 px-2 font-bold border border-slate-300 w-12">S.No.</th>
              <th className="py-1 px-2 font-bold border border-slate-300">Particular</th>
              <th className="py-1 px-2 font-bold border border-slate-300 w-16">Qty</th>
              <th className="py-1 px-2 font-bold border border-slate-300 w-24">Amount</th>
              <th className="py-1 px-2 font-bold border border-slate-300 w-24">Net Amt.</th>
            </tr>
          </thead>
          <tbody>
            {invoice.items.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-4 text-center text-slate-400 border border-slate-300">No services billed.</td>
              </tr>
            ) : (
              invoice.items.map((item, idx) => (
                <tr key={item.id}>
                  <td className="py-1 px-2 border border-slate-300">{idx + 1}</td>
                  <td className="py-1 px-2 border border-slate-300 font-medium">{item.itemName}</td>
                  <td className="py-1 px-2 border border-slate-300">{item.quantity}</td>
                  <td className="py-1 px-2 border border-slate-300">{item.unitPrice.toFixed(0)}</td>
                  <td className="py-1 px-2 border border-slate-300">{item.amount.toFixed(0)}</td>
                </tr>
              ))
            )}
          </tbody>
        ...
        <div className="font-serif text-base mb-2">Receipt Details</div>
        <table className="w-full text-left text-[11px] mb-2 border-collapse">
          <thead>
            <tr>
              <th className="py-1 px-2 font-bold border border-slate-300">Mode</th>
              <th className="py-1 px-2 font-bold border border-slate-300 leading-tight">Bank<br/>Name</th>
              <th className="py-1 px-2 font-bold border border-slate-300 leading-tight">Transaction<br/>No</th>
              <th className="py-1 px-2 font-bold border border-slate-300 leading-tight">Transaction<br/>Type</th>
              <th className="py-1 px-2 font-bold border border-slate-300 leading-tight">Receipt Date Time</th>
              <th className="py-1 px-2 font-bold border border-slate-300">Amount</th>
              <th className="py-1 px-2 font-bold border border-slate-300 leading-tight">Receipt/Refund<br/>No</th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td className="py-1 px-2 border border-slate-300">{paymentMode.toUpperCase()}</td>
              <td className="py-1 px-2 border border-slate-300">{paymentMode !== 'Cash' ? bankName : ''}</td>
              <td className="py-1 px-2 border border-slate-300">{paymentMode !== 'Cash' ? transactionId : ''}</td>
              <td className="py-1 px-2 border border-slate-300">Paid Settlement</td>
              <td className="py-1 px-2 border border-slate-300">{orderDateTime}</td>
              <td className="py-1 px-2 border border-slate-300">{receivedAmt.toFixed(0)}</td>
        ...
        {/* Receipt Totals */}
        <div className="flex flex-col items-end text-[11px] font-serif pr-2 mb-10">
          <div><span className="font-bold">Total Amount </span>{totals.subtotal.toFixed(1)}</div>
          <div><span className="font-bold">Payable Amount </span>{finalAmount.toFixed(1)}</div>
          <div><span className="font-bold">Paid Amount </span>{receivedAmt.toFixed(1)}</div>
          <div><span className="font-bold">Refunded Amount </span>0</div>
        </div>

        {/* Signatures */}
        <div className="flex justify-between text-[11px] font-serif pr-8">
          <div>
            <div>Printed By</div>
        ...
        {/* Printable Footer */}
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#7cb36f] text-white text-center space-y-1 mx-4 mb-4">
          <div className="text-[11px] font-bold tracking-widest uppercase">CHANGING THE WAY OF HEALTHCARE</div>
